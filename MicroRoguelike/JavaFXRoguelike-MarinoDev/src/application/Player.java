...
public class Player extends GridEntity{
	...
	public void Move(int X,int Y) {
        if(canMove) {
        	...
        	if(X < 8 && X > -1 && Y < 8 && Y > -1) {//Player is still in the current room
        		if(GameManager.GetInstance().GetCurrentLevel().GetCurrentRoom().getTileAt(X, Y) != 'W') {
            		SetPosition(X,Y);
            		
            		if(GameManager.GetInstance().GetCurrentLevel().GetCurrentRoom().getTileAt(X, Y) == 'E' && GameManager.GetInstance().isKeyHeld()) {
	...
	public void Interact() {
		...
		List<GridEntity> entities = GameManager.GetInstance().GetCurrentLevel().GetCurrentRoom().GetEntities();
		for (GridEntity e : entities) {
			e.Interact(GetX(),GetY());
		}
	}
